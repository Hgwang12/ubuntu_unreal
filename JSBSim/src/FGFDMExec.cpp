...
// C-string API implementations to avoid std::string ABI issues across DLL boundaries
bool FGFDMExec::LoadModel(const char* model, bool addModelToPath) {
    return LoadModel(std::string(model), addModelToPath);
}
bool FGFDMExec::SetEnginePath(const char* path) {
    return SetEnginePath(SGPath(path));
}
bool FGFDMExec::SetAircraftPath(const char* path) {
    return SetAircraftPath(SGPath(path));
}
bool FGFDMExec::SetSystemsPath(const char* path) {
    return SetSystemsPath(SGPath(path));
}
bool FGFDMExec::SetOutputPath(const char* path) {
    return SetOutputPath(SGPath(path));
}
void FGFDMExec::SetRootDir(const char* rootDir) {
    SetRootDir(SGPath(rootDir));
}
const char* FGFDMExec::GetModelNameCString(void) const {
